  ...
  return (
    <section className="border-t border-cream/10 bg-cream text-night">
      <div className="mx-auto grid max-w-5xl md:grid-cols-2">
        <div className="flex flex-col items-start justify-center gap-5 px-6 py-16 sm:px-10 md:py-24">
          <p className="text-[10px] uppercase tracking-[0.25em] text-night/50 sm:text-xs">
            {copy.resumeCta.label}
          </p>
          <h2 className="font-serif text-3xl italic md:text-5xl">{copy.resumeCta.heading}</h2>
          <p className="max-w-md text-sm leading-relaxed text-night/70 md:text-base">
            {copy.resumeCta.body}
          </p>
          <Link
            to="/resume"
            className="group mt-2 flex items-center gap-2 rounded-full bg-night py-1.5 pl-5 pr-1.5 text-sm font-bold text-cream transition-all hover:gap-3"
          >
            ...
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-cream transition-transform group-hover:scale-110">
              <FileDown className="h-4 w-4 text-night" strokeWidth={1.5} />
            </span>
          </Link>
        </div>
        ...
        <div className="relative min-h-[300px] overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute left-1/2 top-1/2 h-[320px] w-[230px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-5 shadow-[0_24px_60px_rgba(10,15,28,0.25)]"
          >
